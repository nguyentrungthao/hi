...
#elif defined(ESP32)
DWIN::DWIN(HardwareSerial &port, uint8_t receivePin, uint8_t transmitPin, long baud) : listenerCallback(NULL),
                                                                                       _baudrate(baud),
                                                                                       _rxPin(receivePin),
                                                                                       _txPin(transmitPin)
{
    ...
    while ((millis() - startTime < CMD_READ_TIMEOUT))
    {
        while (_dwinSerial->available() > 0)
        {
            ...
            if (data[0] == 0x5A && data[1] == 0xA5 && data[3] == 0x83 &&
                ((uint16_t)data[4] << 8 | (uint16_t)data[5]) == vpAddress)
            {
...
void DWIN::resetGraph(int channel)
{
    ...
    _dwinSerial->write(sendBuffer, sizeof(sendBuffer) / sizeof(sendBuffer[0]));
    // readDWIN();
    flushSerial();
    clearSerial();
}
